...
- 用户认可儿童神经女声版本后，希望再增加一个“语速慢一点点、发音更加饱满”的版本。
- 用户最终选择较年轻的成年女声，并接受新版本不再保持儿童音色。
- Ava 首轮使用理论参数 `-10%` 时，整篇只有 92.528 秒，反而明显快于儿童版的 118.4 秒。

### 根因

1. 不同神经声线的基础语速不同；`-5%` 和 `-10%` 只是相对各自声线的调节值，不能跨声线直接比较。
2. Ava 的基础节奏明显快于 Ana，因此只看负数百分比会误判实际听感速度。
3. 复制第二套全文生成代码会让原稿、停顿和文件规则逐渐漂移，不适合继续扩展声线。

### 处理

1. 把生成器收敛为命名配置：
   - `natural-child` 保持 `AnaNeural / -5% / +0Hz / +0%`
   - `slow-full-young-adult` 使用 `AvaNeural / -33% / -2Hz / +2%`
2. 两套配置共用同一份原稿、10 个自然段、PCM 拼接和 MP3 导出逻辑，只改变合成参数和输出文件名。
3. 用同一代表段落实测：Ana `-5%` 为 17.66 秒，Ava `-10%` 为 14.09 秒，Ava `-30%` 为 18.10 秒，Ava `-33%` 为 18.91 秒。
4. 选择 Ava `-33%` 后重新原生合成，不做时间拉伸、低频 EQ 或动态压缩。
5. 新文件使用独立的 `Fish-Shop-American-Young-Woman-Slow-Full` 文件名，不覆盖儿童版。

### 验证

- 测试先行：新增配置和路由测试先出现 4 项失败，最小实现后 7 项 Python 测试转绿。
- 语速校准也执行红绿测试：断言先从 `-10%` 改为 `-33%` 并确认失败，再修改配置至通过。
- 新 WAV 与 MP3 时长均为 121.928 秒，比儿童版慢约 3%。
- 新 WAV 为单声道 16-bit PCM、24000 Hz；峰值 22572，最后 250 ms 均方根能量为 0。
- 儿童版 WAV 与 MP3 哈希仍分别为 `3451C584...E5DC` 和 `18BE3A38...902F`。
- 后续执行完整 Python 测试、`npm run lint`、`npm run test`、`npm run build` 和 `git diff --check`。

### 后续提醒

- 跨声线调整“快一点 / 慢一点”时，必须比较同段或整篇实测时长，不能只比较 rate 参数。
- 新增声线应继续使用命名配置，不要复制整篇原稿或音频拼接代码。
- 自动校验只能证明参数、文本、编码和信号正常；“更饱满”仍需用户试听确认。
...
- 音频 URL 在线上可以返回，WAV 文件也不是静音，但用户在 iPad 上点击播放仍没有声音。
- 桌面浏览器对延迟调用 `audio.play()` 更宽松，因此之前的桌面播放检查没有暴露这个问题。

### 根因

1. 点击播放后，代码先 `await preloadAudioUrl(url)`，等 `loadeddata`、`canplaythrough` 或超时后才调用 `audio.play()`。
2. 第一次加载或自动播放仍在预热时，这个等待会跨过异步事件任务；iPad Safari 因而不再把后续 `play()` 视为用户点击直接触发，并可能按自动播放策略拒绝。
3. 页面已有独立后台预热，播放函数再次等待预热既重复，也把用户手势授权置于风险中。

### 处理

1. 普通词、慢速词和题库本地音频的播放函数不再等待预热完成。
2. 点击路径在获得 URL 后立即设置播放器并调用 `play()`，确保仍处于用户点击调用栈内。
3. 保留页面后台预热、请求竞态编号和现有失败提示，不改音频资源与播放速度。

### 验证

- 新增回归测试，用不会完成加载的假音频模拟首次加载；修复前点击函数同步阶段 `play()` 调用次数为 0，修复后普通、慢速和题库音频均立即调用。
- `npm run test -- tests/phonetic.test.ts`
- 后续执行完整 `npm run lint`、`npm run test`、`npm run build`。

### 后续提醒

- iOS 媒体播放必须尽量在点击处理函数里同步触发，不能先等待网络、预加载或定时器。
- 预热可以在页面进入后后台执行，但手动播放函数不要依赖预热 Promise 完成。
